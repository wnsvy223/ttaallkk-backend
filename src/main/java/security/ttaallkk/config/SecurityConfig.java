    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
            ...
            .anonymous()
                ...
                .and()
            .authorizeRequests()
                .antMatchers(HttpMethod.POST, "/api/user/signUp", "/api/user/login", "/api/user/refreshToken", "/api/post/**", "/api/comment/**", "/api/like/**", "/api/unlike/**","/api/category/**", "/api/friend/**").permitAll()//회원가입, 로그인, 리프래시토큰발급 앤드포인트는 인증없이 허용.
                .antMatchers(HttpMethod.GET, "/api/user/search/**", "/api/post/**", "/api/comment/**", "/api/like/**", "/api/unlike/**", "/api/category/**", "/api/friend/**").permitAll() //유저 검색 앤드포인트 허용
